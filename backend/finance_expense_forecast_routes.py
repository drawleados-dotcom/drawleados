...
class ReplaceForecastItemsPayload(BaseModel):
    items: List[ForecastItemIn]
...
@expense_forecast_router.put("/{forecast_id}")
async def replace_forecast_items(forecast_id: str, payload: ReplaceForecastItemsPayload, request: Request):
    """Edit a whole day's plan at once — replaces its entire items list
    (rows can be renamed/re-amounted/added/removed together), unlike the
    POST endpoint above which only ever appends."""
    await _get_user(request)
