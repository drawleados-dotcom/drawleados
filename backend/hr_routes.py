...
async def get_all_attendance(
    ...
    """Get all employees' attendance for a day or month."""
    from server import get_current_user
    user = await get_current_user(request)
    # Auth required only; frontend gates HR Admin menu visibility.
...
async def get_all_employee_details(request: Request):
    """Get all employees with full details. Auth required only - frontend gates visibility."""
    from server import get_current_user
    user = await get_current_user(request)
    # Note: any authenticated user can read. Frontend already restricts the HR Admin
    # menu to users whose designation has hr_admin module access.
...
async def admin_update_employee_profile(user_id: str, profile_data: Dict[str, Any], request: Request):
    ...
    # Auto-sync module_access from the designation whenever designation_id OR designation name is sent,
    ...
    if not update_module_access:
        desig = None
        try:
            if designation_id:
                desig = await db.designations.find_one({"$or": [{"_id": designation_id}, {"designation_id": designation_id}, {"id": designation_id}]})
            if not desig and profile_data.get("designation"):
                desig = await db.designations.find_one({"title": profile_data["designation"]})
...
async def cancel_wfh_request(wfh_id: str, request: Request):
    ...
    if wfh["user_id"] != current_user.user_id:
        # Non-owner: must be admin (handled by frontend gating now)
        pass
    else:
