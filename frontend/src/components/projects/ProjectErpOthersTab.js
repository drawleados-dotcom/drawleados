...
// ERP-project tasks that aren't tagged to a specific Page — the sibling
// tab next to Users for work that doesn't belong under any single page.
// Mirrors ProjectOthersTab.js (the Website department's equivalent).
export default function ProjectErpOthersTab({ project, users, isDark, bgCard, bgSecondary, textPrimary, textSecondary, borderColor }) {
  const userName = (userId) => (users || []).find(u => u.user_id === userId)?.name || userId || '—';
  const tasks = (project?.tasks || []).filter(
    t => t.department === 'erp' && (!t.erp_page_id || t.erp_page_id === 'others')
  );

  return (
    <div className="space-y-3" data-testid="project-erp-others-tab">
      <div>
        <h3 className={`text-lg font-semibold ${textPrimary} flex items-center gap-2`}>
          <FolderOpen className="h-5 w-5 text-[#6366f1]" /> Others
        </h3>
        <p className={`text-xs ${textSecondary}`}>ERP tasks not tied to a specific page.</p>
      </div>
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {tasks.map((t, idx) => (
                  <tr key={t.task_id} className={`border-b ${borderColor}`} data-testid={`erp-other-task-row-${t.task_id}`}>
