...
/**
 * Add/Edit popup for a task on the project's Integrations tab — same
 * auto-save architecture as PageTaskModal (debounced PUT/POST as you type,
 * flushed on close), but simpler: no Page/Section scoping, and Task Type
 * is always "Integration" rather than a pickable list, so there's no Task
 * Type field to show at all. Tagged with department: 'integration' so the
 * Integrations tab's own task list (project.tasks filtered on that) finds
 * it, distinct from the website department's Page/Section-tagged tasks.
 */
export default function IntegrationTaskModal({
  ...
  return (
    // z-40: the selects below portal to document.body at z-50 (ui/select.jsx).
      ...
      <div className={`${bgCard} border ${borderColor} rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto`} onClick={(e) => e.stopPropagation()}>
        <div className={`p-5 border-b ${borderColor} flex items-center justify-between`}>
          <h3 className={`text-base font-semibold ${textPrimary} flex items-center gap-2`}>
            <Plug className="h-4 w-4 text-[#6366f1]" /> {taskId ? 'Edit Task' : 'Add Task'}
          ...
          <button onClick={flushAndClose} className={textSecondary} data-testid="integration-quicktask-form-close">
            <X className="h-5 w-5" />
          </button>
        </div>
        <div className="p-5 space-y-4">
          <div>
            ...
            <Textarea
              ...
              placeholder="e.g. Connect Stripe webhook"
              rows={1}
              className={`${bgSecondary} border ${borderColor} ${textPrimary} min-h-[36px] py-2 resize-none overflow-hidden leading-normal`}
              data-testid="integration-quicktask-form-name"
              autoFocus
            />
          </div>
          <div>
            <p className={`text-xs font-medium ${textSecondary} mb-1`}>Details</p>
            <Textarea
              ...
              data-testid="integration-quicktask-form-description"
            />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              >
                <SelectTrigger className={`${bgSecondary} border ${borderColor} ${textPrimary}`} data-testid="integration-quicktask-form-priority">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div>
              ...
              <Input
                ...
                data-testid="integration-quicktask-form-deadline"
              />
            </div>
          </div>
          <div>
            ...
            >
              <SelectTrigger className={`${bgSecondary} border ${borderColor} ${textPrimary}`} data-testid="integration-quicktask-form-assignee">
          ...
          <div>
            ...
            <p className={`text-sm ${textPrimary}`}>Integration</p>
          </div>
        </div>
        <div className={`p-5 border-t ${borderColor} flex items-center justify-end gap-2`}>
          ...
          <Button
            ...
            data-testid="integration-quicktask-form-save"
          >
            Save
          </Button>
        </div>
