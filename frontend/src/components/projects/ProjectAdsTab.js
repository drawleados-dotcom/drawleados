...
export default function ProjectAdsTab({
  ...
  const saveDates = async () => {
    ...
    setSaving(true);
    // Date of Creation (created_at) is tracking only — the "New Ads" count —
    // and is left untouched here; only an explicit Start/End Date gates reporting.
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[70] p-4" onClick={() => setDatesModal(null)}>
          <div className={`${bgCard} border ${borderColor} rounded-xl w-full max-w-sm`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-5 space-y-3">
              ...
              <p className={`text-[11px] ${textSecondary}`}>Reports for this ad are only expected on or after the start date, and stop being expected after the end date. Leave both blank for no restriction.</p>
            </div>
            <div className={`p-5 border-t ${borderColor} flex items-center justify-end gap-2`}>
