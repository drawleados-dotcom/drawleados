...
const StatusBadge = ({ status }) => {
  const map = {
    draft: { label: 'Draft', cls: 'bg-gray-200 dark:bg-[#3f3f46] text-gray-600 dark:text-[#a1a1aa]' },
    sent: { label: 'Sent', cls: 'bg-blue-100 dark:bg-[#1e3a8a]/30 text-blue-600 dark:text-[#60a5fa]' },
    paid: { label: 'Paid', cls: 'bg-emerald-100 dark:bg-[#14532d]/40 text-green-600 dark:text-[#4ade80]' },
    partial: { label: 'Partial', cls: 'bg-amber-100 dark:bg-[#78350f]/40 text-amber-600 dark:text-[#fbbf24]' },
    overdue: { label: 'Overdue', cls: 'bg-red-100 dark:bg-[#7f1d1d]/40 text-red-600 dark:text-[#f87171]' },
    cancelled: { label: 'Cancelled', cls: 'bg-gray-200 dark:bg-[#3f3f46] text-gray-600 dark:text-[#a1a1aa]' },
  };
...
const SummaryCard = ({ icon: Icon, label, value, accent, testId }) => (
  <div
    className="bg-gray-50 dark:bg-[#09090b] border border-gray-200 dark:border-[#27272a] rounded-lg p-4 hover:border-gray-300 dark:hover:border-[#3f3f46] transition-colors"
    data-testid={testId}
  >
    <div className="flex items-center justify-between mb-2">
      <span className="text-xs text-gray-600 dark:text-[#a1a1aa] uppercase tracking-wider">{label}</span>
      <Icon className={`h-4 w-4 ${accent || 'text-[#6366f1]'}`} />
    </div>
    <div className={`text-2xl font-bold ${accent || 'text-gray-900 dark:text-[#fafafa]'}`} style={{ fontFamily: 'Plus Jakarta Sans' }}>
      {value}
    </div>
  </div>
);
      ...
      >
        <DialogHeader>
          ...
          {data?.client?.company_name && (
            <p className="text-sm text-gray-600 dark:text-[#a1a1aa] flex items-center gap-1.5">
          ...
          <div className="space-y-6 mt-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <SummaryCard
                ...
                accent="text-green-600 dark:text-[#4ade80]"
              ...
              <SummaryCard
                ...
                accent="text-red-600 dark:text-[#f87171]"
              ...
              <SummaryCard
                ...
                accent="text-amber-600 dark:text-[#fbbf24]"
                testId="summary-invoice-count"
              />
            </div>

            {/* Contact + Tax Info */}
            <div className="bg-gray-50 dark:bg-[#09090b] border border-gray-200 dark:border-[#27272a] rounded-lg p-5">
              <h3 className="text-base font-semibold mb-3">Client Details</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                {data.client.email && (
                  <div className="flex items-center gap-2 text-gray-700 dark:text-[#d4d4d8]">
                    <Mail className="h-3.5 w-3.5 text-gray-600 dark:text-[#a1a1aa]" />
                    <span className="text-gray-600 dark:text-[#a1a1aa]">Email:</span> {data.client.email}
                  </div>
                )}
                {(data.client.mobile || data.client.work_phone) && (
                  <div className="flex items-center gap-2 text-gray-700 dark:text-[#d4d4d8]">
                    <Phone className="h-3.5 w-3.5 text-gray-600 dark:text-[#a1a1aa]" />
                    <span className="text-gray-600 dark:text-[#a1a1aa]">Phone:</span> {data.client.mobile || data.client.work_phone}
                  </div>
                )}
                {data.client.gstin && (
                  <div className="flex items-center gap-2 text-gray-700 dark:text-[#d4d4d8]">
                    <CreditCard className="h-3.5 w-3.5 text-gray-600 dark:text-[#a1a1aa]" />
                    <span className="text-gray-600 dark:text-[#a1a1aa]">GSTIN:</span> {data.client.gstin}
                  </div>
                )}
                {data.client.place_of_supply && (
                  <div className="flex items-center gap-2 text-gray-700 dark:text-[#d4d4d8]">
                    <MapPin className="h-3.5 w-3.5 text-gray-600 dark:text-[#a1a1aa]" />
                    <span className="text-gray-600 dark:text-[#a1a1aa]">Place of Supply:</span> {data.client.place_of_supply}
                  </div>
                )}
              </div>
            </div>
            ...
            {/* === TABS === */}
            <div className="border-b border-gray-200 dark:border-[#27272a] flex items-center gap-1 overflow-x-auto">
            ...
            {activeTab === 'services' && (
              <div className="space-y-3" data-testid="client-services-pane">
                <p className="text-xs text-gray-600 dark:text-[#a1a1aa]">
                  ...
                  <span className="text-violet-600 dark:text-[#a78bfa]"> Operations → Projects</span>.
                </p>
                {projectsLoading ? (
                  <div className="flex justify-center py-6"><Loader2 className="h-5 w-5 animate-spin text-[#6366f1]" /></div>
                ) : projects.length === 0 ? (
                  <div className="bg-gray-50 dark:bg-[#09090b] border border-gray-200 dark:border-[#27272a] rounded-lg p-8 text-center text-gray-600 dark:text-[#a1a1aa]">
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {projects.map((p) => (
                          <tr
                            key={p.project_id}
                            className="border-t border-gray-200 dark:border-[#27272a] hover:bg-gray-50/50 dark:hover:bg-[#1c1c1f]/50"
                          ...
                          >
                            <td className="px-4 py-2 font-medium">
                              <div className="flex flex-col">
                                <span>{p.name || '—'}</span>
                                {p.description && (
                                  <span className="text-xs text-gray-600 dark:text-[#a1a1aa] line-clamp-1">{p.description}</span>
                                )}
                              </div>
                            </td>
                            <td className="px-4 py-2 text-gray-600 dark:text-[#a1a1aa]">
                              {(p.departments || []).length > 0
                                ? (p.departments || []).join(', ')
                                : '—'}
                            </td>
                            <td className="px-4 py-2 text-gray-600 dark:text-[#a1a1aa]">{fmtDate(p.start_date)}</td>
                            <td className="px-4 py-2 text-gray-600 dark:text-[#a1a1aa]">{fmtDate(p.due_date)}</td>
                            <td className="px-4 py-2 text-right font-medium">
                              {fmtCurrency(p.contract_total, data.client.currency)}
                            </td>
                            <td className="px-4 py-2 text-center">
                              <span className="px-2 py-0.5 rounded text-xs font-medium bg-emerald-100 dark:bg-[#14532d]/40 text-green-600 dark:text-[#4ade80]">
                                {p.status || 'active'}
                              </span>
                            </td>
                          </tr>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {paymentSchedule.map((r, i) => (
                          <tr key={`${r.split_id || i}`} className="border-t border-gray-200 dark:border-[#27272a] hover:bg-gray-50/50 dark:hover:bg-[#1c1c1f]/50" data-testid={`ps-row-${i}`}>
                            <td className="px-4 py-2 font-medium">{r.project_name || '—'}</td>
                            <td className="px-4 py-2 text-gray-700 dark:text-[#d4d4d8]">{r.label || '—'}</td>
                            <td className="px-4 py-2 text-gray-600 dark:text-[#a1a1aa]">{fmtDate(r.due_date)}</td>
                            <td className="px-4 py-2 text-right font-medium">{fmtCurrency(r.amount, data.client.currency)}</td>
                            <td className="px-4 py-2 text-gray-600 dark:text-[#a1a1aa]">{r.invoice_number || '—'}</td>
                            <td className="px-4 py-2 text-center">
                              {r.collected ? (
                                <span className="px-2 py-0.5 rounded text-xs font-medium bg-emerald-100 dark:bg-[#14532d]/40 text-green-600 dark:text-[#4ade80]">Collected</span>
                              ) : (
                                <span className="px-2 py-0.5 rounded text-xs font-medium bg-amber-100 dark:bg-[#78350f]/40 text-amber-600 dark:text-[#fbbf24]">Pending</span>
                              )}
                            </td>
                          </tr>
                        ))}
                ...
                {fbLoading ? (
                  ...
                  <div className="space-y-2">
                    {feedback.map((f) => (
                      <div
                        key={f.feedback_id}
                        className="bg-gray-50 dark:bg-[#09090b] border border-gray-200 dark:border-[#27272a] rounded-lg p-4"
                            ...
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {f.rating && (
                                <div className="flex items-center">
                                  {[1, 2, 3, 4, 5].map((n) => (
                                    <Star key={n} className={`h-3.5 w-3.5 ${n <= f.rating ? 'fill-amber-500 dark:fill-[#fbbf24] text-amber-600 dark:text-[#fbbf24]' : 'text-gray-400 dark:text-[#3f3f46]'}`} />
                                  ))}
                                </div>
                              )}
                              <span className="text-xs text-gray-600 dark:text-[#a1a1aa]">{fmtDate(f.created_at)}</span>
                            </div>
                            <p className="text-sm text-gray-700 dark:text-[#d4d4d8] whitespace-pre-wrap">{f.comment}</p>
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            <div>
              <h3 className="text-base font-semibold mb-3 flex items-center gap-2">
                <Calendar className="h-4 w-4 text-violet-600 dark:text-[#a78bfa]" /> Invoice History
              </h3>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {data.invoices.map((inv) => (
                        <tr key={inv.invoice_id} className="border-t border-gray-200 dark:border-[#27272a] hover:bg-gray-50/50 dark:hover:bg-[#1c1c1f]/50">
                          <td className="px-4 py-2 font-medium">{inv.invoice_number}</td>
                          <td className="px-4 py-2 text-gray-600 dark:text-[#a1a1aa]">{fmtDate(inv.invoice_date)}</td>
                          <td className="px-4 py-2 text-gray-600 dark:text-[#a1a1aa]">{fmtDate(inv.due_date)}</td>
                          <td className="px-4 py-2 text-right font-medium">
                            {fmtCurrency(inv.total_amount, data.client.currency)}
                          </td>
                          <td className="px-4 py-2 text-right text-green-600 dark:text-[#4ade80]">
